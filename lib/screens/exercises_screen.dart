...
class _ExercisesScreenState extends State<ExercisesScreen> {
  ...
  Future<void> _create() async {
    final result = await _showEditor(title: 'New exercise');
  ...
  Future<void> _edit(CatalogEntry entry) async {
    final result = await _showEditor(title: 'Edit exercise', entry: entry);
    if (result == null) return;
  ...
  /// Shared create/edit form (name, description, default sets/reps/max).
  Future<_ExerciseFormResult?> _showEditor({
    required String title,
    CatalogEntry? entry,
  }) {
    final nameCtrl = TextEditingController(text: entry?.name ?? '');
    final descCtrl = TextEditingController(text: entry?.description ?? '');
    final setsCtrl = TextEditingController(text: entry?.defaultSets?.toString() ?? '');
    final repsCtrl = TextEditingController(text: entry?.defaultReps?.toString() ?? '');
    final maxCtrl = TextEditingController(text: entry?.defaultRepsMax?.toString() ?? '');
    ...
    return showPaperDialog<_ExerciseFormResult>(
      context: context,
      builder: (context) => Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          _formLabel('Name'),
          _formField(nameCtrl, autofocus: entry == null),
          const SizedBox(height: 8),
          _formLabel('Description'),
          _formField(descCtrl, maxLines: 2, hint: 'form cues, notes…'),
          const SizedBox(height: 10),
          _formLabel('Default sets × reps'),
          Row(
            crossAxisAlignment: CrossAxisAlignment.end,
            children: [
              _smallNum(setsCtrl, 'sets'),
              _glyphText('×'),
              _smallNum(repsCtrl, 'reps'),
              _glyphText('to'),
              _smallNum(maxCtrl, 'max'),
            ],
          ),
            ...
            children: [
              ...
              PenButton(
                ...
                onPressed: () {
                  ...
                  var mx = parseInt(maxCtrl);
                  final reps = parseInt(repsCtrl);
